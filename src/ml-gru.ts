...
export const trainModel = async (
  ...
  console.log(
    `Labels: ${labels.length} one-hot vectors of length ${labels[0].length}`
  );
  ...
  const xTensor = tf.tensor3d(
    features,
    [features.length, SEQ_LEN, NUM_AXES],
    "float32"
  );
...
const createModel = (
  ...
  console.log(
    "Building model: GRU units=",
    modelOptions.neuronNumber,
    "dropoutRate=",
    modelOptions.dropoutRate,
    "optimizer=Adam lr=",
  ...
  const gru1 = tf.layers
    .gru({ units: modelOptions.neuronNumber, returnSequences: true })
    .apply(input) as SymbolicTensor;
  const drop1 = tf.layers
    .dropout({ rate: modelOptions.dropoutRate })
    .apply(gru1) as SymbolicTensor;
  const gru2 = tf.layers
    .gru({ units: modelOptions.neuronNumber })
    .apply(drop1) as SymbolicTensor;
  const drop2 = tf.layers
    .dropout({ rate: modelOptions.dropoutRate })
    .apply(gru2) as SymbolicTensor;

  // Optional hidden dense
  const hidden = tf.layers
    .dense({ units: modelOptions.neuronNumber, activation: "relu" })
    .apply(drop2) as SymbolicTensor;
